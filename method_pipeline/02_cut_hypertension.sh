...
GWAS_FILE="/common/lir2lab/Wanling/FDR_AB_PRS/External_PRS/consortium_GWAS/WojcikG_PMID_htn"
OUTPUT_DIR="/common/lir2lab/Olivia/Model-Evaluation/consortium/result/sumstats/hypertension/"
...
do
    ...
    OUTPUT_FILE="${OUTPUT_DIR}hypertension_chr${i}_gwas"
    mkdir -p "$(dirname "$OUTPUT_FILE")"

    # Extract the header, replace column names, and write to output
    head -1 "$GWAS_FILE" | sed 's/\<Beta\>/BETA/g; s/\<P-val\>/pval/g; s/\<SNP\>/Makername/g; s/\<rsid\>/SNP/g' > "$OUTPUT_FILE"
