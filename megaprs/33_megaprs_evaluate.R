...
out_dir <- '/common/lir2lab/Olivia/Model-Evaluation/consortium/result/megaprs/'
#phenos <- c("t2d", "breast_cancer", "hdl", "ldl", "bmi", "cholesterol")
#binaries <- c(TRUE, TRUE, FALSE, FALSE, FALSE, FALSE) 
...
test_scores <- data.frame(matrix(ncol = length(phenos), nrow = 1))
...
for (pheno_idx in seq_along(phenos)) {  
    ...
    file_dir <- paste0(out_dir, pheno, '/')
    score_sum <- fread(paste0(file_dir, 'score_train/megaprs-sum.txt'))
